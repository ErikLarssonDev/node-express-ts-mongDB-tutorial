...
export class BuyerService {
  ...
  async checkout(userId: string, cardToken: string, userEmail: string) {
    ...
    if (!cart) return new BadRequestError("the cart is empty");
    if (cart.products.length === 0)
      return new BadRequestError("the cart is empty");
    ...
    const charge = await this.stripeService.charges.create({
      amount: cart.totalPrice * 100, // Need to send the amount i cents
      currency: "usd",
  ...
  async updateCustomerStripeCard(userId: string, newCardToken: string) {
    ...
    if (!cart.customer_id) return new BadRequestError("your're not a customer");
