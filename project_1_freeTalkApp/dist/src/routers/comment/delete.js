...
router.delete("/api/comment/:commentId/delete/:postId", (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!commentId || !postId) {
        const error = new Error("Comment id and post id are required!");
        error.status = 400;
        next(error);
    }
    try {
        yield comment_1.default.findOneAndRemove({ _id: commentId }); // Better to use remove than delete. The document will stay in a cache in the storage in the mongoDB.
    }
    catch (err) {
